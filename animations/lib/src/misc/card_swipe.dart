...
class CardSwipeDemo extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _CardSwipeDemoState();
}
...
class SwipeableCard extends StatefulWidget {
  final String imageAssertName;
  final VoidCallback onSwiped;

  const SwipeableCard({
    required this.onSwiped,
    required this.imageAssertName,
    super.key,
  });

  @override
  State<StatefulWidget> createState() => _SwipeableCardState();
}
...
class _SwipeableCardState extends State<SwipeableCard>
  ...
  Widget build(BuildContext context) {
    return SlideTransition(
      ...
      child: GestureDetector(
        ...
        child: Card(imageAssetName: widget.imageAssertName),
      ),
    );
  }
  ...
  void _dragUpdate(DragUpdateDetails detail) {
    print('_dragUpdate');
    var isSwipingLeft = (detail.localPosition.dx - _dragStartX) < 0;
    if (isSwipingLeft != _isSwipingLeft) {
      _isSwipingLeft = isSwipingLeft;
      _updateAnimation(detail.localPosition.dx);
    }
    ...
    setState(() {
      ...
      _controller.value =
          (detail.localPosition.dx - _dragStartX).abs() / size.width;
    });
  }
